...
public class SmartInventory {
    ...
    oldInv.ifPresent(
        inv -> {
          inv.getListeners().stream()
              ...
              .forEach(
                  listener ->
                      ((InventoryListener<InventoryCloseEvent>) listener)
                          .accept(new InventoryCloseEvent(player.getOpenInventory())));

          this.manager.setInventory(player, null);
        });

    InventoryLayout effectiveLayout =
        this.layout != null ? this.layout : new RectangularLayout(this.rows, this.columns);
    InventoryContents contents =
        new InventoryContents.Impl(this, player.getUniqueId(), effectiveLayout);
  ...
  public Optional<InventoryLayout> getLayout() {
