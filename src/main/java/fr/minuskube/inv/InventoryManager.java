...
public class InventoryManager {
  ...
  public void handleInventoryOpenError(
      SmartInventory inventory, Player player, Exception exception) {
    inventory.close(player);

    Bukkit.getLogger().log(Level.SEVERE, "Error while opening SmartInventory:", exception);
  }

  public void handleInventoryUpdateError(
      SmartInventory inventory, Player player, Exception exception) {
    inventory.close(player);
    ...
    public void onInventoryClick(InventoryClickEvent e) {
      ...
      if (clickedInventory == p.getOpenInventory().getTopInventory()) {
        ...
        InventoryLayout layout =
            inv.getLayout().orElse(new RectangularLayout(inv.getRows(), inv.getColumns()));
          ...
          .forEach(
              (player, inv) -> {
                inv.getListeners().stream()
                    ...
                    .forEach(
                        listener -> ((InventoryListener<PluginDisableEvent>) listener).accept(e));
